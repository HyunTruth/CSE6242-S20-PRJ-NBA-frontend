...
```{r}
rolling <- rollify(mean,window = 8,na_value = 0)
...
Running_DFGP <- GameLogs %>%
  ...
  group_by(Team,Game_Date) %>%
  summarise(DFGM = sum(DFGM), DFGA = sum(DFGA)) %>%
  mutate(Running_DFGM = cumsum(DFGM),
         Running_DFGA = cumsum(DFGA),
         DFG_Percentage = round(Running_DFGM/Running_DFGA,2))

hchart(Running_DFGP, "spline", hcaes(x = "Game_Date", y = "DFG_Percentage",
                                        name = "Team", Date  = "Game_Date", 
                                        DFGP = "DFG_Percentage")) %>%
  hc_tooltip(pointFormat = "<b>{point.name}</b><br />Date: {point.Date}<br />DFG%: {point.DFGP}") %>%
  hc_title(text = "DFG% Season Trend") %>%
  hc_subtitle(text = "NBA 2019-2020 Season") %>%
  hc_credits(enabled = TRUE,
             text = "data via stats.nba.com",
